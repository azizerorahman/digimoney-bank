...
import { 
  Search, User, FileText, TrendingUp, AlertTriangle, 
  CheckCircle, XCircle, Clock, DollarSign, Calendar,
  Upload, Download, Mail, Phone, Eye, Edit,
  BarChart3, PieChart, ArrowUp, ArrowDown, Filter,
  CreditCard, Building, Home, Car, GraduationCap,
  Shield, Flag, MessageSquare, Paperclip, Star, MapPin,Briefcase,Users
} from 'lucide-react';
...
const NewApplicationModal = ({ isOpen, onClose }) => {
  ...
  const handleRiskReview = () => {
    ...
    setActiveTab('risk');
    alert('Risk Review - Switching to Risk Assessment tab with flagged applications');
  };

  // Filter applications
// NEW - KEEP THIS ONE
const filteredApplications = loanOfficerData.loanApplications.filter(app => {
  // First filter by status and loan type
  const statusMatch = filterStatus === 'all' || app.status.toLowerCase().includes(filterStatus.toLowerCase());
  const typeMatch = filterLoanType === 'all' || app.loanType.toLowerCase().includes(filterLoanType.toLowerCase());
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                  ...
                                  <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm mb-6">
                                    <div className="border-b border-gray-200 dark:border-gray-700">
                                      <nav className="flex space-x-8 px-6">
                                        {[
                                          { id: 'pipeline', label: 'Application Pipeline', icon: FileText },
                                          { id: 'credit', label: 'Credit Analysis', icon: TrendingUp },
                                          { id: 'risk', label: 'Risk Assessment', icon: Shield },
                                          { id: 'portfolio', label: 'Loan Portfolio', icon: PieChart },
                                          ...
                                          <button
                                            ...
                                            className={`flex items-center space-x-2 py-4 px-1 border-b-2 font-medium text-sm ${
                                              activeTab === tab.id
                                                ...
                                                : 'border-transparent text-black dark:text-white hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600'
                                            }`}
                                          >
                                      ...
                                      {activeTab === 'pipeline' && (
                                        <div className="space-y-6">
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                                            {[
                                              { status: 'New Applications', count: 8, color: 'bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-400' },
                                              { status: 'In Review', count: 15, color: 'bg-yellow-100 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-400' },
                                              { status: 'Pending Approval', count: 6, color: 'bg-purple-100 dark:bg-purple-900/20 text-purple-800 dark:text-purple-400' },
                                              { status: 'Approved', count: 23, color: 'bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-400' },
                                              { status: 'Flagged', count: 3, color: 'bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-400' }
                                            ].map((item, index) => (
                                              <div key={index} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-center">
                                                ...
                                                <div className="text-2xl font-bold text-black dark:text-white">{item.count}</div>
                                              </div>
                                            ))}
                                          </div>
                                              ...
                                              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                <thead className="bg-gray-50 dark:bg-gray-700">
                                                  <tr>
                                                    ...
                                                    <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Loan Details</th>
                                                    <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Credit Score</th>
                                                    <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Risk Level</th>
                                                    <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Status</th>
                                                    <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">SLA</th>
                                                    ...
                                                    <tr key={application.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                      <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="flex items-center">
                                                          <div>
                                                            <div className="text-sm font-medium text-black dark:text-white">{application.applicantName}</div>
                                                            <div className="text-sm text-black dark:text-white">{application.id}</div>
                                                          </div>
                                                        </div>
                                                      </td>
                                                      <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="flex items-center space-x-2">
                                                          {getLoanTypeIcon(application.loanType)}
                                                          <div>
                                                            <div className="text-sm font-medium text-black dark:text-white">{application.loanType}</div>
                                                            <div className="text-sm text-black dark:text-white">${application.amount.toLocaleString()}</div>
                                                          </div>
                                                        </div>
                                                      </td>
                                                      <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="flex items-center space-x-2">
                                                          <span className="text-sm font-medium text-black dark:text-white">{application.creditScore}</span>
                                                          {application.creditTrend === 'up' ? (
                                                            <ArrowUp className="h-4 w-4 text-green-500" />
                                                          ) : application.creditTrend === 'down' ? (
                                                            <ArrowDown className="h-4 w-4 text-red-500" />
                                                      ...
                                                      <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="text-sm text-black dark:text-white">
                                                          <div className={`font-medium ${
                                                            application.daysInProcess >= 7 ? 'text-red-600 dark:text-red-400' :
                                                            application.daysInProcess >= 5 ? 'text-yellow-600 dark:text-yellow-400' :
                                                            'text-green-600 dark:text-green-400'
                                                          }`}>
                                                            {application.daysInProcess} days
                                                          </div>
                                                          <div className="text-xs">{application.slaDeadline}</div>
                                                        </div>
                                                      </td>
                                        ...
                                        <div className="space-y-6">
                                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                            ...
                                            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                                              <h3 className="text-lg font-semibold text-black dark:text-white mb-4">Credit Score Analysis - John Smith</h3>
                                              <div className="space-y-4">
                                                <div className="flex items-center justify-between">
                                                  <span className="text-sm text-black dark:text-white">Current Score</span>
                                                  <span className="text-2xl font-bold text-blue-600 dark:text-blue-400">742</span>
                                                </div>
                                                
                                                {/* Credit Score History Chart Placeholder */}
                                                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 h-48 flex items-center justify-center">
                                                  <div className="text-center">
                                                    ...
                                                    <p className="text-sm text-black dark:text-white">Credit Score Trend Chart</p>
                                                    <p className="text-xs text-black dark:text-white">6-month history showing upward trend</p>
                                                  </div>
                                                </div>
                          
                                                {/* Credit Factors */}
                                                <div className="space-y-3">
                                                  <h4 className="font-medium text-black dark:text-white">Credit Factors</h4>
                                                  {loanOfficerData.creditScoreData["APP-2024-001"].factors.map((factor, index) => (
                                                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                                      <div>
                                                        <span className="text-sm font-medium text-black dark:text-white">{factor.factor}</span>
                                                        <div className="text-xs text-black dark:text-white">{factor.score}% utilization</div>
                                                      </div>
                                            ...
                                            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                                              <h3 className="text-lg font-semibold text-black dark:text-white mb-4">Credit Bureau Reports</h3>
                                              <div className="space-y-4">
                                                {['Experian', 'Equifax', 'TransUnion'].map((bureau, index) => (
                                                  <div key={index} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                                    <div className="flex items-center space-x-3">
                                                      ...
                                                      <div>
                                                        <div className="font-medium text-black dark:text-white">{bureau}</div>
                                                        <div className="text-sm text-black dark:text-white">Score: {742 + (index * 5)}</div>
                                                      </div>
                                                    </div>
                                              ...
                                              <div className="mt-6">
                                                <h4 className="font-medium text-black dark:text-white mb-3">Recent Credit Activity</h4>
                                                <div className="space-y-2">
                                                  <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border-l-4 border-green-400 dark:border-green-500">
                                                    <div>
                                                      <span className="text-sm font-medium text-green-800 dark:text-green-400">Payment Posted</span>
                                                      <div className="text-xs text-green-700 dark:text-green-300">Credit card payment of $1,250 - May 28</div>
                                                    </div>
                                                    <CheckCircle className="h-4 w-4 text-green-600 dark:text-green-400" />
                                                  </div>
                                                  <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border-l-4 border-blue-400 dark:border-blue-500">
                                                    <div>
                                                      <span className="text-sm font-medium text-blue-800 dark:text-blue-400">Credit Inquiry</span>
                                                      <div className="text-xs text-blue-700 dark:text-blue-300">Mortgage inquiry from First National Bank - May 15</div>
                          </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                  <h3 className="text-lg font-semibold text-black dark:text-white mb-4">Credit Score Benchmarking</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="text-center">
                      <div className="text-3xl font-bold text-green-600 dark:text-green-400">742</div>
                      <div className="text-sm text-black dark:text-white">Applicant Score</div>
                      <div className="text-xs text-green-600 dark:text-green-400 mt-1">Excellent Range</div>
                    </div>
                    <div className="text-center">
                      <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">680</div>
                      <div className="text-sm text-black dark:text-white">Portfolio Average</div>
                      <div className="text-xs text-blue-600 dark:text-blue-400 mt-1">Above Average</div>
                    </div>
                    <div className="text-center">
                      <div className="text-3xl font-bold text-purple-600 dark:text-purple-400">650</div>
                      <div className="text-sm text-black dark:text-white">Industry Minimum</div>
                      <div className="text-xs text-purple-600 dark:text-purple-400 mt-1">Well Above Min</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-black dark:text-white">Total Portfolio</p>
                        <p className="text-2xl font-bold text-black dark:text-white">$5.2M</p>
                      </div>
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-black dark:text-white">Active Loans</p>
                        <p className="text-2xl font-bold text-black dark:text-white">47</p>
                      </div>
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-black dark:text-white">Delinquent</p>
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-black dark:text-white">Avg Interest Rate</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <h3 className="text-lg font-semibold text-black dark:text-white mb-4">Portfolio by Loan Type</h3>
                    <div className="space-y-4">
                      {[
                        { type: 'Mortgages', amount: '$3.2M', percentage: 62, count: 18 },
                        { type: 'Auto Loans', amount: '$1.4M', percentage: 27, count: 23 },
                        { type: 'Personal Loans', amount: '$450K', percentage: 8, count: 5 },
                        { type: 'Business Loans', amount: '$150K', percentage: 3, count: 1 }
                      ].map((item, index) => (
                        <div key={index} className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            {getLoanTypeIcon(item.type)}
                            <div>
                              <div className="text-sm font-medium text-black dark:text-white">{item.type}</div>
                              <div className="text-xs text-black dark:text-white">{item.count} loans</div>
                            </div>
                          </div>
                          <div className="text-right">
                            <div className="text-sm font-medium text-black dark:text-white">{item.amount}</div>
                            <div className="text-xs text-black dark:text-white">{item.percentage}%</div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-black dark:text-white">On-Time Payment Rate</span>
                        <span className="text-sm font-medium text-green-600 dark:text-green-400">94.2%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-black dark:text-white">Default Rate</span>
                        <span className="text-sm font-medium text-red-600 dark:text-red-400">1.8%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-black dark:text-white">Early Payment Rate</span>
                        <span className="text-sm font-medium text-blue-600 dark:text-blue-400">12.5%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-black dark:text-white">Portfolio Yield</span>
                        <span className="text-sm font-medium text-purple-600 dark:text-purple-400">4.8%</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* Active Loans Table */}
                <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                  <h3 className="text-lg font-semibold text-black dark:text-white mb-4">Active Loans</h3>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Borrower</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Loan Details</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Balance</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Payment</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Status</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Next Due</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {loanOfficerData.activeLoanPortfolio.map((loan, index) => (
                          <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div>
                                <div className="text-sm font-medium text-black dark:text-white">{loan.borrower}</div>
                                <div className="text-sm text-black dark:text-white">{loan.loanId}</div>
                              </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center space-x-2">
                                ...
                                <div>
                                  <div className="text-sm font-medium text-black dark:text-white">{loan.type}</div>
                                  <div className="text-sm text-black dark:text-white">{loan.interestRate}% • {loan.termRemaining}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm font-medium text-black dark:text-white">${loan.currentBalance.toLocaleString()}</div>
                              <div className="text-sm text-black dark:text-white">of ${loan.originalAmount.toLocaleString()}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm font-medium text-black dark:text-white">${loan.monthlyPayment.toLocaleString()}</div>
                              <div className="text-sm text-black dark:text-white">monthly</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(loan.status)}`}>
                                {loan.status}
                              </span>
                              <div className="text-xs text-black dark:text-white mt-1">{loan.paymentHistory}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-black dark:text-white">{loan.nextPaymentDue}</div>
                              <div className={`text-xs ${
                                loan.status === 'Late' ? 'text-red-600 dark:text-red-400' : 'text-black dark:text-white'
                              }`}>
                                {loan.status === 'Late' ? 'OVERDUE' : 'On Schedule'}
                              </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                              <button className="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 mr-3">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-black dark:text-white">Today's Contacts</p>
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-black dark:text-white">Pending Responses</p>
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-black dark:text-white">Follow-ups Due</p>
                        <p className="text-2xl font-bold text-red-600 dark:text-red-400">3</p>
                      </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <h3 className="text-lg font-semibold text-black dark:text-white mb-4">Quick Communication</h3>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-black dark:text-white mb-2">Select Applicant</label>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-black dark:text-white mb-2">Communication Type</label>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-black dark:text-white mb-2">Template</label>
                        <select className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-black dark:text-white">
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    <h3 className="text-lg font-semibold text-black dark:text-white mb-4">Communication Templates</h3>
                    <div className="space-y-3">
                      {[
                        {
                        ...
                        <div key={index} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer">
                          <div className="flex justify-between items-start">
                            <div>
                              <div className="font-medium text-black dark:text-white">{template.title}</div>
                              <div className="text-sm text-black dark:text-white">{template.description}</div>
                              <div className="text-xs text-black dark:text-white mt-1">{template.usage}</div>
                            </div>
                            <button className="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">
                              <Edit className="h-4 w-4" />
                            </button>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                  <h3 className="text-lg font-semibold text-black dark:text-white mb-4">Recent Communications</h3>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Date/Time</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Applicant</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Type</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Subject</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Status</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-black dark:text-white uppercase tracking-wider">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {[
                          ...
                          <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-black dark:text-white">{comm.date}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm font-medium text-black dark:text-white">{comm.applicant}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center space-x-2">
                                {comm.type === 'Email' && <Mail className="h-4 w-4 text-blue-600 dark:text-blue-400" />}
                                {comm.type === 'Phone' && <Phone className="h-4 w-4 text-green-600 dark:text-green-400" />}
                                {comm.type === 'SMS' && <MessageSquare className="h-4 w-4 text-purple-600 dark:text-purple-400" />}
                                <span className="text-sm text-black dark:text-white">{comm.type}</span>
                              </div>
                            </td>
                            <td className="px-6 py-4">
                              <div className="text-sm text-black dark:text-white">{comm.subject}</div>
                              <div className="text-xs text-black dark:text-white">{comm.direction}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                  <h3 className="text-lg font-semibold text-black dark:text-white mb-4">Scheduled Follow-ups</h3>
                  <div className="space-y-3">
                    {[
                      {
                        applicant: "David Chen",
                      ...
                      <div key={index} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border-l-4 border-blue-400 dark:border-blue-500">
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="font-medium text-black dark:text-white">{followUp.applicant}</div>
                            <div className="text-sm text-black dark:text-white">{followUp.application}</div>
                            <div className="text-sm text-black dark:text-white mt-1">{followUp.followUp}</div>
                          </div>
                          <div className="text-right">
                            <div className="text-sm text-black dark:text-white">{followUp.dueDate}</div>
